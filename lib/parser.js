...
Parser.prototype.parse = function(s){
	for(var i = 0; i < s.length; i++){
		var c = s[i];
		if(this._escaping && this._enclosing && c === this.enclosedChar){
			this._escaping = !this._escaping;
			this._text = this._text + c;
		} else if(this.escapeChar === c && this._enclosing && i+1 === s.length){
			this._escaping = true;
		} else if(this.escapeChar === c && this._enclosing && s[i+1] === this.enclosedChar){
			i++;
